...
export function loadFromLocalStorage<T>(key: string, defaultOrClass: T | (new (data: any) => T)): T {
  ...
  try {
    ...
    // Kiểm tra nếu raw có chứa từ "system" không phải JSON hợp lệ
    if (raw.includes('system') && !raw.startsWith('{') && !raw.startsWith('[') && !raw.startsWith('"')) {
      console.warn(`Phát hiện dữ liệu localStorage không hợp lệ cho key "${key}":`, raw);
      localStorage.removeItem(key); // Xóa dữ liệu hỏng
      return defaultOrClass instanceof Function ? new defaultOrClass({}) : defaultOrClass;
    }
