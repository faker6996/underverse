...
export default function DataTableServerExample() {
  ...
  const columns: DataTableColumn<User>[] = [
    ...
    { key: "actions", title: "Actions", render: (_, record) => (
      <button className="text-primary hover:underline" onClick={() => alert(`Edit ${record.name}`)}>Edit</button>
  ...
  React.useEffect(() => { fetchServer(lastQuery); }, []);
  ...
  page={page} pageSize={pageSize} onQueryChange={fetch} />`} />
    </div>
  );
}
