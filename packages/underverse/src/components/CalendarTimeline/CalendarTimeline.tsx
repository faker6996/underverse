...
export default function CalendarTimeline<TResourceMeta = unknown, TEventMeta = unknown>({
  ...
  const formatToken = React.useCallback((key: string, params: Record<string, string | number>) => {
    let message = t(key);
  ...
  const l: Required<CalendarTimelineLabels> = React.useMemo(
    () => ({
      ...
      more: labels?.more ?? ((n) => formatToken("more", { n })),
  ...
  const title = React.useMemo(() => {
    ...
    if (activeView === "sprint") {
      ...
      return `${l.sprint} • ${yearText} • ${formatToken("sprints", { n: count })}`;
    }
